...
def adjust_SSC_forward(work_dir, info_file):
    save_dir = os.path.join(os.path.dirname(work_dir), 'SSC_adj')
...
def adjust_start_to_LSC(work_dir, info_text):
    save_dir = os.path.join(os.path.dirname(work_dir), 'LSC_adj')
...
def adjust_start_to_RP(work_dir, info_text):
    save_dir = os.path.join(os.path.dirname(work_dir), 'RP_adj')
    info_cont = open(info_text, 'r')
    info_line = info_cont.readline()
        # check save directory exist or not
    if not os.path.exists(save_dir):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--work_dir", help="Input directory of fasta files")
    ...
    if args.work_dir and args.info_file:
        # Assuming the user specifies which function to call via an additional command line argument
        if args.mode == 'SSC':
            adjust_SSC_forward(args.work_dir, args.info_file)
        elif args.mode == 'LSC':
            adjust_start_to_LSC(args.work_dir, args.info_file)
        elif args.mode == 'RP':
            adjust_start_to_RP(args.work_dir, args.info_file)
        else:
